...
function () {
  function Mario(game, x, y, luigi) {
    ...
    this.game.mario = this; // spritesheet

    this.spritesheet = ASSET_MANAGER.getAsset("./sprites/mario.png");
    if (luigi) this.spritesheet = ASSET_MANAGER.getAsset("./sprites/luigi.png"); // mario's state variables
    ...
    this.dead = false; // fire mario's state variables
    ...
    this.fallAcc = 562.5;
    this.updateBB(); // mario's animations
  ...
  _createClass(Mario, [{
    ...
    value: function loadAnimations() {
      ...
      this.animations[0][2][0] = new Animator(this.spritesheet, 209, 122, 16, 32, 1, 0.33, 14, false, true); // facing left = 1
      ...
      this.animations[0][2][1] = new Animator(this.spritesheet, 180, 122, 16, 32, 1, 0.33, 14, false, true); // walk animation
      // facing right
      ...
      this.animations[1][2][0] = new Animator(this.spritesheet, 237, 122, 16, 32, 3, 0.10, 9, true, true); // facing left
      ...
      this.animations[1][2][1] = new Animator(this.spritesheet, 102, 122, 16, 32, 3, 0.10, 9, false, true); // run animation
      // facing right
      ...
      this.animations[2][2][0] = new Animator(this.spritesheet, 237, 122, 16, 32, 3, 0.05, 9, true, true); // facing left
      ...
      this.animations[2][2][1] = new Animator(this.spritesheet, 102, 122, 16, 32, 3, 0.05, 9, false, true); // slide animation
      // facing right
      ...
      this.animations[3][2][0] = new Animator(this.spritesheet, 337, 122, 16, 32, 1, 0.33, 14, false, true); // facing left
      ...
      this.animations[3][2][1] = new Animator(this.spritesheet, 52, 122, 16, 32, 1, 0.33, 14, false, true); // jump animation
      // facing right
      ...
      this.animations[4][2][0] = new Animator(this.spritesheet, 362, 122, 16, 32, 1, 0.33, 9, true, true); // facing left
      ...
      this.animations[4][2][1] = new Animator(this.spritesheet, 27, 122, 16, 32, 1, 0.33, 9, false, true); // duck animation
      // facing right
      ...
      this.animations[5][2][0] = new Animator(this.spritesheet, 389, 117, 16, 32, 1, 0.33, 14, false, true); // facing left
      ...
      this.animations[5][2][1] = new Animator(this.spritesheet, 0, 118, 16, 32, 1, 0.33, 14, false, true); // pulling animation
      // facing right
      ...
      this.animations[6][2][0] = new Animator(this.spritesheet, 361, 157, 15, 32, 2, 0.12, 14, false, true); // facing left
    ...
    value: function updateBB() {
      if (this.size === 0 || this.size === 3) {
        ...
        if (this.game.down) // big mario is crouching
          this.BB = new BoundingBox(this.x, this.y + PARAMS.BLOCKWIDTH, PARAMS.BLOCKWIDTH, PARAMS.BLOCKWIDTH);else this.BB = new BoundingBox(this.x, this.y, PARAMS.BLOCKWIDTH, PARAMS.BLOCKWIDTH * 2);
      }
    }
  }, {
    key: "updateLastBB",
    value: function updateLastBB() {
      this.lastBB = this.BB;
    }
  }, {
    key: "die",
    ...
    value: function update() {
      var TICK = this.game.clockTick; // I used this page to approximate my constants
      // https://web.archive.org/web/20130807122227/http://i276.photobucket.com/albums/kk21/jdaster64/smb_playerphysics.png
      // I converted these values from hex and into units of pixels and seconds.

      var MIN_WALK = 4.453125;
      var MAX_WALK = 93.75;
      var MAX_RUN = 153.75;
      var ACC_WALK = 133.59375;
      var ACC_RUN = 200.390625;
      var DEC_REL = 182.8125;
      var DEC_SKID = 365.625;
      var MIN_SKID = 33.75;
      var STOP_FALL = 1575;
      var WALK_FALL = 1800;
      var RUN_FALL = 2025;
      var STOP_FALL_A = 450;
      var WALK_FALL_A = 421.875;
      var RUN_FALL_A = 562.5;
      var MAX_FALL = 270;
      this.fireballsThrown = 0;
      ...
      if (this.dead) {
        ...
        if (this.y > PARAMS.BLOCKWIDTH * 16) {
          ...
          if (this.game.camera.lives < 0) this.game.camera.gameOver = true;else this.game.camera.loadLevel(levelOne, 2.5 * PARAMS.BLOCKWIDTH, 0 * PARAMS.BLOCKWIDTH, true, false);
        }
      } else if (this.state === 6) {
        var BLOCK_TOP = 13 * PARAMS.BLOCKWIDTH;
        var MARIO_SLIDE_SPEED = 12;
        var FLAG_WIDTH = PARAMS.BLOCKWIDTH / 7;
        var MARIO_FLIP_DELAY = 1.75;
        var MARIO_JUMP_DELAY = 0.5;
        var BASE_FLAG_HEIGHT = BLOCK_TOP - PARAMS.BLOCKWIDTH;
        /*
        ...
        */
        ...
        if (!this.game.camera.title) {
          if (this.state !== 4) {
            ...
            if (this.win) {
              this.velocity.x = 50; // if (this.x === ??) delay for .5 secs, disappear
        ...
        this.velocity.y += this.fallAcc * TICK; // max speed calculation
        ...
        if (this.velocity.x <= -MAX_WALK && !this.game.B) this.velocity.x = -MAX_WALK; // update position

        this.x += this.velocity.x * TICK * PARAMS.SCALE;
        this.y += this.velocity.y * TICK * PARAMS.SCALE;
        this.updateLastBB();
        this.updateBB(); // if mario fell of the map he's dead

        if (this.y > PARAMS.BLOCKWIDTH * 16) this.die(); // collision
        ...
        this.game.entities.forEach(function (entity) {
          if (entity.BB && that.BB.collide(entity.BB)) {
            if (that.velocity.y > 0) {
              ...
              && !entity.dead) {
                // can't squish an already squished Goomba
                entity.dead = true;
              ...
              that.lastBB.top >= entity.BB.bottom) {
                ...
                if (that.BB.collide(entity.leftBB) && that.BB.collide(entity.rightBB)) {
                  ...
                  if (entity.type == 1 && that.size != 0 && that.size != 3) {
                    // if it's a regular brick, and mario is big
                    entity.explode();
                  }
        ...
        if (this.state !== 4 && this.state !== 6) {
          if (this.game.down) this.state = 5;else if (Math.abs(this.velocity.x) > MAX_WALK) this.state = 2;else if (Math.abs(this.velocity.x) >= MIN_WALK) this.state = 1;else this.state = 0;
    ...
    value: function draw(ctx) {
      ...
      } else if (this.size == 2 && this.game.B && this.throwFireballTimeElapsed < 0.1) {
        // throwing fireballs
        if (this.facing == 0) {
