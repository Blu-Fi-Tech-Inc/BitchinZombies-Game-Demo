...
function () {
  function SceneManager(game) {
    ...
    this.loadLevel(levelOne, 2.5 * PARAMS.BLOCKWIDTH, 13 * PARAMS.BLOCKWIDTH, false, true); // NOTE: PLEASE USE THE FOLLOWING LINE TO TEST.
  ...
  _createClass(SceneManager, [{
    ...
    key: "loadLevel",
    value: function loadLevel(level, x, y, transition, title) {
      this.title = title;
      this.level = level;
      this.clearEntities();
      this.x = 0;
      ...
      if (transition) {
        ...
        if (level.hills) {
          for (var i = 0; i < level.hills.length; i++) {
            var _hill = level.hills[i];
            this.game.addEntity(new Hill(this.game, _hill.x * PARAMS.BLOCKWIDTH, _hill.y * PARAMS.BLOCKWIDTH));
          }
        }

        if (level.bushes) {
          for (var i = 0; i < level.bushes.length; i++) {
            var bush = level.bushes[i];
        ...
        if (level.koopaParatroopaGreen) {
          for (var i = 0; i < level.koopaParatroopaGreen.length; i++) {
            var koopaParatroopaGreen = level.koopaParatroopaGreen[i];
        ...
        if (level.koopaParatroopaRed) {
          for (var i = 0; i < level.koopaParatroopaRed.length; i++) {
            var koopaParatroopaRed = level.koopaParatroopaRed[i];
    ...
    value: function update() {
      ...
      var midpoint = PARAMS.CANVAS_WIDTH / 2 - PARAMS.BLOCKWIDTH / 2;
      if (this.x < this.mario.x - midpoint) this.x = this.mario.x - midpoint; // NOTE: THIS FOLLOWING CODE HAS A BUG WHERE CANVAS COLOR WON'T CHANGE BACK TO BLUE.
    ...
    value: function draw(ctx) {
      ...
      if (this.title && !this.credits) {
        ...
        ctx.drawImage(ASSET_MANAGER.getAsset("./sprites/mouse_wheel.png"), 10.75 * PARAMS.BLOCKWIDTH, 5.9 * PARAMS.BLOCKWIDTH, 30, 30); // Credits box text output

        ctx.font = '15px "Press Start 2P"';
        var j = 0;

        for (var i = this.creditsLineIndex; i < credits.text.length && i < this.creditsLineIndex + 12; i++) {
      ...
      if (PARAMS.DEBUG) {
        var xV = "xV=" + Math.floor(this.game.mario.velocity.x);
        var yV = "yV=" + Math.floor(this.game.mario.velocity.y);
        ctx.fillText(xV, 1.5 * PARAMS.BLOCKWIDTH, 2.5 * PARAMS.BLOCKWIDTH);
        ctx.fillText(yV, 1.5 * PARAMS.BLOCKWIDTH, 3 * PARAMS.BLOCKWIDTH);
        ctx.fillText("FPS ".concat(this.game.timer.ticks.length), 12.5 * PARAMS.BLOCKWIDTH, 2.5 * PARAMS.BLOCKWIDTH);
